...
typedef enum {
    T_NUMBER, T_PLUS, T_MINUS, T_STAR, T_SLASH, T_POW, T_LPAREN, T_RPAREN, T_EOF
} TokenType;
...
static void tv_push(TV *v, Token t){
    if (v->n==v->cap){ v->cap = v->cap? v->cap*2 : 64; v->a = realloc(v->a, v->cap*sizeof(Token)); }
    v->a[v->n++] = t;
}
...
static void build_default_outdir(const char *input_path, char *out, size_t n){
    ...
    const char *sid  = getenv_or("STUDENT_ID","231ADB119");
...
static void build_output_filename(const char *input_path, char *out, size_t n){
    const char *slash = strrchr(input_path, SEP[0]);
    const char *base = slash? slash+1 : input_path;
    const char *nm  = getenv_or("NAME","Yigit");
    const char *ln  = getenv_or("LASTNAME","Yildiz");
    const char *sid = getenv_or("STUDENT_ID","231ADB119");
